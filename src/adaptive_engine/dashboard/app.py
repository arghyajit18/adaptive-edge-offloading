...
from adaptive_engine.dashboard.components import empty_figure, metric_card
...
app.layout = html.Div(
    ...
    children=[
        ...
        dcc.Interval(
            id="tick", interval=settings.dashboard.refresh_interval_sec * 1000, n_intervals=0
        ),
    ],
)
...
def update_network_graphs(_n, last_ts):
    ...
    fig_bw.update_layout(
        template="plotly_white", yaxis_title="Mbps", margin={"l": 30, "r": 10, "t": 30, "b": 30}
    )
    fig_rtt = go.Figure(go.Scatter(x=df["timestamp"], y=df["rtt_ms"], mode="lines+markers"))
    fig_rtt.update_layout(
        template="plotly_white", yaxis_title="ms", margin={"l": 30, "r": 10, "t": 30, "b": 30}
    )
    fig_loss = go.Figure(go.Scatter(x=df["timestamp"], y=df["loss"], mode="lines+markers"))
    fig_loss.update_layout(
        template="plotly_white", yaxis_title="Loss", margin={"l": 30, "r": 10, "t": 30, "b": 30}
    )
    fig_sinr = go.Figure(go.Scatter(x=df["timestamp"], y=df["sinr_db"], mode="lines+markers"))
    fig_sinr.update_layout(
        template="plotly_white", yaxis_title="dB", margin={"l": 30, "r": 10, "t": 30, "b": 30}
    )
...
def update_decision_graphs(_n, last_ts):
    ...
    fig_ratio = go.Figure(
        go.Pie(labels=ratio.index.tolist(), values=ratio.values.tolist(), hole=0.4)
    )
    fig_ratio.update_layout(template="plotly_white", margin={"l": 10, "r": 10, "t": 30, "b": 10})
    fig_lat = go.Figure(
        go.Scatter(x=recent["timestamp"], y=recent["predicted_latency_ms"], mode="lines+markers")
    )
    fig_lat.update_layout(
        template="plotly_white", yaxis_title="ms", margin={"l": 30, "r": 10, "t": 30, "b": 30}
    )
    fig_en = go.Figure(
        go.Scatter(x=recent["timestamp"], y=recent["predicted_energy_mj"], mode="lines+markers")
    )
    fig_en.update_layout(
        template="plotly_white", yaxis_title="mJ", margin={"l": 30, "r": 10, "t": 30, "b": 30}
    )
    ...
    fig_miss.update_layout(
        template="plotly_white", yaxis_title="% Miss", margin={"l": 30, "r": 10, "t": 30, "b": 30}
    )
